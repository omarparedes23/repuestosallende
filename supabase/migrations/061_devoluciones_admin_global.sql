-- 061: administradores sin sucursal_id operan globalmente dentro de su empresa.
BEGIN;

ALTER FUNCTION public.ra_solicitar_devolucion_v1(uuid,uuid,jsonb,text)
  RENAME TO ra_solicitar_devolucion_v1_060;
CREATE FUNCTION public.ra_solicitar_devolucion_v1(p_operation_id uuid,p_venta_id uuid,p_items jsonb,p_motivo text)
...
BEGIN
  ...
  RETURN public.ra_solicitar_devolucion_v1_060(p_operation_id,p_venta_id,p_items,p_motivo);
END $$;
...
ALTER FUNCTION public.ra_aprobar_devolucion_v1(uuid,uuid,boolean,text)
  RENAME TO ra_aprobar_devolucion_v1_059;
CREATE FUNCTION public.ra_aprobar_devolucion_v1(p_operation_id uuid,p_devolucion_id uuid,p_reingreso_aprobado boolean,p_reingreso_override_motivo text default null)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,extensions,pg_temp AS $$
...
BEGIN
  ...
  RETURN public.ra_aprobar_devolucion_v1_059(p_operation_id,p_devolucion_id,p_reingreso_aprobado,p_reingreso_override_motivo);
END $$;
...
ALTER FUNCTION public.ra_liquidar_devolucion_v1(uuid,uuid,jsonb)
  RENAME TO ra_liquidar_devolucion_v1_060;
CREATE FUNCTION public.ra_liquidar_devolucion_v1(p_operation_id uuid,p_devolucion_id uuid,p_referencias jsonb default '{}'::jsonb)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public,extensions,pg_temp AS $$
DECLARE v_user uuid:=auth.uid(); v_empresa uuid; v_sucursal uuid; v_devolucion_sucursal uuid;
BEGIN
  IF v_user IS NULL THEN RAISE EXCEPTION USING MESSAGE='RA_UNAUTHENTICATED'; END IF;
  ...
  SELECT sucursal_id INTO v_devolucion_sucursal FROM public.ra_devoluciones WHERE id=p_devolucion_id AND empresa_id=v_empresa FOR SHARE;
  IF v_devolucion_sucursal IS NULL OR (v_sucursal IS NOT NULL AND v_devolucion_sucursal IS DISTINCT FROM v_sucursal) THEN RAISE EXCEPTION USING MESSAGE='RA_NOT_FOUND'; END IF;
  RETURN public.ra_liquidar_devolucion_v1_060(p_operation_id,p_devolucion_id,p_referencias);
END $$;
