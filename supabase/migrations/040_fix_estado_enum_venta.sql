-- Fix forward-only: cast explicito de ra_estado_venta en ra_confirmar_venta_v1.
-- Bug: el INSERT de cabecera usaba un CASE con literales text para la columna
-- estado (enum ra_estado_venta). PostgreSQL no coacciona text->enum implicitamente
-- dentro de un CASE, y la venta fallaba con:
--   ERROR: column "estado" is of type ra_estado_venta but expression is of type text
-- Reemplaza unicamente el cuerpo con el cast explicito en ambos brazos del CASE.
-- No cambia firma, permisos, ni otros objetos.
