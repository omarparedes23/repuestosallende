...
BEGIN
  ...
  UPDATE public.ra_series_documento SET serie=CASE serie WHEN 'BCTST' THEN 'BC01' WHEN 'FCTST' THEN 'FC01' WHEN 'BCTS' THEN 'BC01' WHEN 'FCTS' THEN 'FC01' END,updated_at=now()
  WHERE empresa_id=e AND sucursal_id=s AND tipo_documento IN ('nota_credito_boleta','nota_credito_factura') AND serie IN ('BCTST','FCTST','BCTS','FCTS');
  INSERT INTO public.ra_series_documento(empresa_id,sucursal_id,tipo_documento,serie,siguiente_correlativo,activo,es_predeterminada) VALUES(e,s,'nota_credito_boleta','BC01',1,true,true),(e,s,'nota_credito_factura','FC01',1,true,true) ON CONFLICT (empresa_id,sucursal_id,tipo_documento,serie) DO UPDATE SET activo=true,es_predeterminada=true,updated_at=now();
