...
BEGIN
  ...
  UPDATE public.ra_series_documento SET serie=CASE serie WHEN 'BCTST' THEN 'BCTS' WHEN 'FCTST' THEN 'FCTS' END,updated_at=now()
  WHERE empresa_id=e AND sucursal_id=s AND tipo_documento IN ('nota_credito_boleta','nota_credito_factura') AND serie IN ('BCTST','FCTST');
  INSERT INTO public.ra_series_documento(empresa_id,sucursal_id,tipo_documento,serie,siguiente_correlativo,activo,es_predeterminada) VALUES(e,s,'nota_credito_boleta','BCTS',1,true,true),(e,s,'nota_credito_factura','FCTS',1,true,true) ON CONFLICT (empresa_id,sucursal_id,tipo_documento,serie) DO UPDATE SET activo=true,es_predeterminada=true,updated_at=now();
